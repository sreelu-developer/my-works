...
`wk_address`, DATE_FORMAT(wk_startdate,'%d/%M/%Y' ) AS `wk_startdate`,
 DATE_FORMAT(wk_finishdate,'%d/%M/%Y' ) AS `wk_finishdate`, `wk_status`, DATE_FORMAT(wk_memberapplydate,'%d/%M/%Y' ) AS `wk_memberapplydate`, `wk_contract`, 
 `wk_employeeapplystrength`, DATE_FORMAT(wk_adsdate,'%d/%M/%Y' ) AS `wk_adsdate`, `wk_adsstatus`, `wk_panchayathdate`,
 `wk_panchayathstatus`, DATE_FORMAT(wk_aedate,'%d/%M/%Y' ) AS `wk_aedate`, 
 `wk_aestatus`, `wk_employeeapprovestrength`,
...
<body>
    <?php
	if($row1['wk_status']=="started" || $row1['wk_status']=="finished"){
		$currentmenu="approved-works";
	}
	else if($row1['wk_status']=="waiting"){
		$currentmenu="waiting-approval";
	}
	else if($row1['wk_status']=="rejected"){
		$currentmenu="rejected-works";
	}
                ...
                <div class="row">
					...
					<div class="card">
							<div class="card-header">
								...
								if($row1['wk_status']=="started" || $row1['wk_status']=="finished"){
								?>
									<a href="list_approved_works.php" class="btn btn-sm btn-outline-secondary pull-right"><i class="fa fa-chevron-left"></i>&nbsp;Back</a>
